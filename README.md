...
**배포 주소: <https://remicon-match-iota.vercel.app>**
휴대폰에서 바로 열립니다. `main` 에 push 하면 Vercel 이 자동으로 다시 배포합니다.

조원에게 공유할 진행 현황 보고서:
[docs/레미go-진행현황-20260925.pdf](docs/레미go-진행현황-20260925.pdf)
발표용 대본: [docs/시연-시나리오.md](docs/시연-시나리오.md)
...
키가 있는데도 **로그인 없이** 화면만 보고 싶으면(조원이 아직 키를 못 받았거나,
화면만 고쳤을 때):

```bash
npm run dev:demo      # http://localhost:3001 — Supabase 키를 뺀 채로 실행
```
...
| 파일 | 고정하는 것 |
| --- | --- |
| `tests/allocate.test.ts` | 지시서 6장 A~G 예시 — **A5·B4·C10·D5·E12·F14·G0**, 이동시간 합 1605분 |
| `tests/predict.test.ts` | KCS 14 20 10 표 3.3-1 — 25℃ 경계, 이어치기 간격과 타설 공백의 차이 |
| `tests/dashboard.test.ts` | 현황판 집계 — 오늘 주문, 출하 간격, 출하 예정 회차, 현장 도착 대기 순번 |
...
| 현장 | `/site` | ★ 현황판 — 오늘·타설량·다음 차량·공백·지연 알림·긴급 배차 요청 |
...
| 레미콘사 | `/plant` | ★ 현황판 — 오늘 출하량·출하 능력·출하 간격·출하 예정·현장별 주문량·배차 현황 |
| 레미콘사 | `/plant/orders` | 주문 수락 / 사유 달아 거절 · ★ 긴급 주문 먼저 |
| 레미콘사 | `/plant/dispatch` | 차량 배차 · 출하 지시(비비기 시작 기록) |
| 기사 | `/driver` | ★ 차량 배정 · ★ 현장 지도·최적 경로 · ★ 도착 대기 순번 · ★ 배차 변경 알림 · 운행 시작(GPS) |
...
supabase/migrations/    0001_init.sql(스키마+RLS) · 0002_seed.sql(시연 데이터) · 0003_urgent.sql(긴급 배차)
...
배분이 불가능하면 "불가"만 말하지 않고 대안을 같이 계산해 내놓습니다.

| 대안 | 언제 나오나 |
| --- | --- |
| **이번 타설을 N m³ 로 줄이기** | 항상. 지금 조건에서 가능한 최대 물량을 이분 탐색으로 찾아 제시하고, 버튼 한 번으로 적용됩니다 |
| 이른 아침(25℃ 미만)으로 옮기기 | 외기 25℃ 이상일 때. 제한이 90분 → 120분이 되어 후보 공장이 늘어납니다 |
| 펌프 속도 낮추기 | 시간당 출하 능력이 모자랄 때. **차 자체가 모자라면 도움이 안 되므로 내놓지 않습니다** |
| 안전 여유 줄이기 | 그것만으로 가능해질 때 |
| 공장에 출하 능력 상향 요청 | 항상 (사람이 해야 하는 일) |
...
| 현장 도착 대기 | 펌프 1대가 `UNLOAD_EST_MIN`(20분)씩 받는다고 보고 줄을 세움 | 실제 하역 기록으로 추정치 교체. 펌프 2대 현장은 아직 못 다룸 |
| 긴급 배차 요청 | 현장이 공장 하나를 골라 보냄 | 여러 공장에 동시에 뿌리고 먼저 수락한 곳이 가져가는 방식 |
| 트럭 경로 | 카카오 `car_type=7`(대형화물) | 높이·중량 제한, 실제 운행 기록으로 보정계수 갱신 |
| 기사 이름 표시 | `미배정`/`배정됨` 으로만 표시 | `profiles` 읽기 정책을 같은 회사까지 넓혀야 함 |
| 기사 GPS 실전 시험 | 시뮬레이터로만 확인 | 휴대폰 2대로 실제 운행 시험 (화면 꺼짐·배터리·신호 끊김) |
